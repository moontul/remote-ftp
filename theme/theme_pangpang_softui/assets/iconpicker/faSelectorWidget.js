...
const FAS = {
	...
	config: {
		...
		search: {
			...
			field_placeholder: "아이콘 이름을 입력해보세요..(알파벳)" // Placeholder text for the selectors search box.
		}
	},
							...
							if (FAS.config.search.search_all_icons || FAS.selectors[selector_id].category == "all") {
								arr[i].styles.forEach(style => {
									...
									//if (FAS.config.show_labels) {
										label = arr[i].label;
									//}
									styles = arr[i].styles[0];

									let aliases
									if(arr[i].aliases){aliases=arr[i].aliases.names;}
									FAS.outputThis(selector,arr[i].svg[style].raw,uni,label,styles,aliases)
								})
							} else {
								let uni;
								let label;
								...
								if (arr[i].svg[FAS.selectors[selector_id].category]) {
									FAS.outputThis(selector,arr[i].svg[FAS.selectors[selector_id].category].raw,uni,label,styles,aliases)
								}
							}
						} else {
							if (FAS.selectors[selector_id].category == "all") {
	...
	outputThis: (selector,svg,uni,label,styles,aliases) => {

		let el = document.createElement("i");

		el.setAttribute('styles', styles);
		el.setAttribute('label', label);
		el.setAttribute('aliases', aliases);

		el.setAttribute("onclick", "FAS.clickicon('"+styles+"','"+label+"')");
	...
	clickicon:(styles, label) => {

		var i="<i class=\"fa"+styles.substring(0,1)+" fa-"+label.toLowerCase().replace(/ /gi,'-')+"\"></i>";

		document.querySelectorAll(".fa-selected")[0].innerHTML=i;
		document.querySelectorAll(".fa-child-container")[0].style.display="none";
