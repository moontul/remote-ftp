...
<!--//cdnjs.cloudflare.com/ajax/libs/three.js/108/three.min.js./space-warp-src/three.min.js  <?=($_SERVER['HTTPS']=="on")?"https:":"http:"?>-->
  <script src="./space-warp-src/three.min.js"></script>
  <script>
  ...
  function init() {
    ...
    let sprite = new THREE.TextureLoader().load( 'space-warp-src/star.png' );
  ...
  <header class="header-2">
    ...
    <?php $bg=rand(0,8)?>
    <div class="page-header min-vh-75" id="spacebackground"
        style="background-image: url('<?php echo G5_THEME_URL?>/assets/img/curved-images/space<?=$bg?>.jpg');"
      >
      <div class="container">
        <div class="row">
          <div class="wrapcanvas"></div>

          <div class="col-lg-7 text-center mx-auto">
            <h1 class="text-white pt-3 mt-n5">IT & Coding  Question Bank System</h1>
...
function test() {
//  console.log("Hello!");
  //$("#spacebackground").css("background-image","url('<?php echo G5_THEME_URL?>/assets/img/curved-images/space1.jpg')");
}
...
<?php
$sql="select count(*) as qcnt, sum(case when is_compiler is not null then 1 end) as ccnt from tb_question where is_del=0";
$result=sql_fetch($sql);
$qcnt=$result["qcnt"];
$ccnt=$result["ccnt"];

$sql="select count(*) as lcnt from tb_container where is_open=1 and type='강좌'";
$result=sql_fetch($sql);
$lcnt=$result["lcnt"];
?>
          ...
          <div class="row">
            <div class="col-md-4 position-relative">
              <div class="p-3 text-center">
                <h1 class="text-gradient text-primary"><span id="state1" countTo="<?=$qcnt?>">0</span>+</h1>
                <h5 class="mt-3"><a href="/page?2022032320274510E549">기출문제</a></h5>
                <p class="text-sm">정보처리기사, 기능사, 필기, 실기 문제를 모았어요</p>
              </div>
              <hr class="vertical dark">
            </div>
            <div class="col-md-4 position-relative">
              <div class="p-3 text-center">
                <h1 class="text-gradient text-primary"> <span id="state2" countTo="<?=$ccnt?>">0</span>+</h1>
                <h5 class="mt-3"><a href="/codingq?Python">코딩 문제</a></h5>
                <p class="text-sm">C, C++, Java, Python 직접 코딩해 볼 수 있는 문제</p>
              </div>
              <hr class="vertical dark">
            </div>
            <div class="col-md-4">
              <div class="p-3 text-center">
                <h1 class="text-gradient text-primary" id="state3" countTo="<?=$lcnt?>">0</h1>
                <h5 class="mt-3"><a href="/page?2022032320225934BDF5">강좌</a></h5>
                <p class="text-sm">쉽고 재미있게 공부할 수 있는 IT 강좌입니다</p>
              </div>
            </div>
          </div>
        </div>
...
-->
<?php } // xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx?>

<hr class="container">
<div style="position:relative;z-index:2;">
  <?php
  $sql="select * from tb_page where list='2022032320225934BDF5'";
  $rs=sql_fetch($sql);
  $page_idx=$rs["idx"];
  ?>
  <div class="d-lg-flex container">
    <div class="col-lg-3">
      <div class="h3"><?=$rs["title"]?></div>
    </div>
    <div class="col-lg-9">
    <?php
      include("page.main.listview.php");
    ?>
    </div>
  </div>
</div>
  ...
  <?php
  $sql="select * from tb_page where list='2022032320274510E549'";
  $rs=sql_fetch($sql);
  $page_idx=$rs["idx"];
  ?>

  <div class="d-lg-flex container py-3 mt-3">
    <div class="col-lg-3">
      <div class="h3"><?=$rs["title"]?></div>
    </div>
    <div class="col-lg-9">
  ...
  <?php
  $sql="select * from tb_page where list='202203232103120000AA'";
  $rs=sql_fetch($sql);
  $page_idx=$rs["idx"];
  ?>
...
<?php if($is_admin){ ?>
  <style>
  section.pp-adm-edit {border:3px dotted pink;}
  </style>
<?php } ?>

  <div class="container">
  <?php @include_once("pagesection/section.index1.ini")?>
  <?php if($is_admin){?>
    <span style="position:absolute;margin-top:-40px;"><a class="" href="page_sectioneditor?f=section.index1" target="_blank"><i class="fa fa-cog"></i></a></span>
  <?php } ?>
  <script>//$(".pp-adm-edit").prepend("<span><i class='fa fa-cog'></i></span>");</script>
  </div>
