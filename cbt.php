...
//리스트가 넘어오면 해당 list가 tb_elist, tb_qlist 인지 검사
$list=$_GET["list"];
...
//전체문제 쿼리 : page_list의 조건을 살표보고 문제를 불러오는 쿼리
...
$arr_qlist=array(); //문제각각에 대한 과목코드

$arr_qtext=array(array()); //질문


$arr_list=array(); //list코드배열
$arr_ttitle=array(); //list제목
...
<div id="unsolved" class="ui-dialog ui-widget ui-widget-content ui-corner-all ui-front ui-draggable" tabindex="-1" role="dialog"
	...
	<form name=f method=post action="cbt_save.php">
		...
		<input type=hidden name=page_list value="<?=$page_list?>">
	...
	<div class="section-tool-block">
				...
				<div class="progress-info-block">
				  <div class="total-count-block">전체 문제 수&nbsp;&nbsp;: <span class="count-info-total"><?=$total_qnum?></span>
					...
					<div class="unsolved-count-block">안 푼 문제 수 : <span class="count-info-unsolved"><?=$total_qnum?></span></div>
				</div>
			</div>

	<div class="test-page-title-block" style="display: block;"><div class="test-page-title-name"><?=$now_title?></div>
...
<input type=hidden name="qlist_a[]" value="<?=$arr_qlist[$x-1]?>">
	...
	<div class="sheet-section-block">
		<select name="sel-section" id="sel-section">
		  ...
		  for($x=0;$x<count($arr_list);$x++){?>
			<option value="<?=$arr_listsnum[$x]?>"><?=$arr_title[$x]?>(<?=$arr_listqcnt[$x]?>문제 <?=$arr_listsnum[$x]?>~<?=$arr_listenum[$x]?>)</option>
			<?}?>
		</select>
	</div>
...
<?php //답안표기란 끝?>
	...
	<div class="footer-block">
	    <div class="footer-left-block">
				<!--
	    		<a button="calculator" id="btnCaculator">
						<img src="/cbtutil/images/btn_calculator.gif" class="calculator_icon"> <span class="unsolved_label">계산기</span>
					</a>
				-->
...
$("#btnLabel").click(function(){
	...
	var cnt=parseInt($(".count-info-unsolved").text());


	if(confirm("답안지를 제출하시겠습니까?")){
		if(cnt>0){
			if(confirm("안 푼 "+cnt+"문제가 있습니다. 그래도 정말 제출할까요?")){
				document.f.submit();
			}
		}else{
			document.f.submit();
