...
for($i=0;$i<count($code_col_all);$i++){
  ${$code_col_all[$i]} = addslashes(trim($_POST[$code_col_all[$i]]));
  echo($code_col_all[$i] . ":".   ${$code_col_all[$i]} ."<br>" );
  if($i>0){$sql_common.=",";}
  $sql_common .= " $code_col_all[$i]     = '${$code_col_all[$i]}'";
}
...
//    @unlink(G5_DATA_PATH."/content/{$co_id}_t");
      ...
      $sql = " delete from $code_table where $code_code = '$code' ";
      echo($sql);
      sql_query($sql);

      $sql= " delete from $code_unittable where $code_code='$code' ";
      echo($sql);
      sql_query($sql);
...
if ($w == "" || $w == "u")
{
  /*
    ...
    */
    goto_url("./code_edit.php?w=u&amp;code=$code");

}
else
{
    goto_url("./code_list.php?code=$code");
}
